...
export class BinanceCredentialsController {
  constructor(private readonly binanceCredentialsService: BinanceCredentialsService) {}

  @Post('check')
  @HttpCode(HttpStatus.OK)
  async check(@Body() checkDto: CheckBinanceCredentialsDto) {
    const permissions = await this.binanceCredentialsService.checkCredentials(checkDto.apiKey, checkDto.secretKey);
  ...
  async save(@Request() req: any, @Body() saveDto: SaveBinanceCredentialsDto) {
    ...
    const result = await this.binanceCredentialsService.saveCredentials(userId, saveDto.apiKey, saveDto.secretKey);
    return result; 
  }
  ...
  async checkToken(@Request() req: any, @Body() body: { masterToken: string }) {
    ...
    const permissions = await this.binanceCredentialsService.checkMasterToken(req.user.id, body.masterToken);
    return { ok: true, permissions };
  }
}
